  ...
  return (
    <>
      ...
      <div className="flex flex-col w-full md:hidden">
        <label htmlFor={props.id} className="text-start text-white mb-2">
          {props.label}
        </label>
        <input
          id={props.id}
          placeholder={props.placeholderMovile}
          {...field}
          type={props.type}
          className="rounded-full text-white focus-within:outline-2 border-white border-2 border-transparent focus:outline-none focus:border-accent focus:placeholder:text-accent placeholder-gray-400 transition-all duration-300  bg-[#C5C5C5]/15 py-4 px-5 backdrop-blur-2xl placeholder:text-white placeholder:font-medium"
        />
        <span
          className={`text-red-500 mt-1 transition-all ${props.error ? " opacity-100 translate-x-0" : " opacity-0 -translate-x-2"}`}
        >
    ...
    <>
      ...
      <div className="flex md:hidden flex-col w-full">
        ...
        <textarea
          ...
          placeholder={props.placeholderMovile}
          rows={6}
          className="rounded-3xl text-white focus-within:outline-2 border-white border-2 border-transparent focus:outline-none focus:border-accent focus:placeholder:text-accent placeholder-gray-400 transition-all duration-300  bg-[#C5C5C5]/15 py-4 px-5 backdrop-blur-2xl placeholder:text-white placeholder:font-medium"
        />
        <span
          className={`text-red-500 mt-1 transition-all ${props.error ? " opacity-100 translate-x-0" : " opacity-0 -translate-x-2"}`}
        >
